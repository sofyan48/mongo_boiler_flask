...
def validate(access_token):
    ...
    Returns:
        dict -- access token validation status
    """
    result = Account_token.objects.get(ldap_token=access_token)

    if result:
        now = arrow.now()
        print(result.ldap_token_expiry)
        token_datetime = arrow.get(result.ldap_token_expiry)
